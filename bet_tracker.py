...
def get_glicks_picks():
    ...
    try:
        ...
        for item in data:
            ...
            picks.append({
                "Event": event, "Price": f"+{raw_p}" if raw_p > 0 else str(raw_p), "Book": clean_book_name(item.get("best_book")),
                "Time": d_time, "SortKey": sort_key, "raw_odds": raw_p,
                "game_pk": item.get("game_pk"), "player_name": p_name, "market": p_mkt, "line": p_line, "dir": p_dir
            })
        return sorted(picks, key=lambda x: x['SortKey'])
    except: return []

# --- 5. DATA STORAGE HELPERS ---
...
if st.session_state["authentication_status"]:
    ...
    if st.session_state.get('pending_track'):
        ...
        st.session_state.autofill_meta = {
            "game_pk": track['game_pk'], "player_name": track['player_name'], 
            "market": track['market'], "line": track['line'], "dir": track['dir'], "odds": track['odds']
        }
    ...
    if nav == "🎯 Picks":
        ...
        for p in picks:
            with st.container(border=True):
                ...
                if c2.button("Track", key=f"track_{p['Event']}"):
        ...
        with st.form("bet_form", clear_on_submit=True):
            ...
            if st.form_submit_button("Save Bet"):
                ...
                st.session_state.autofill_event = ""; st.session_state.autofill_meta = {}; st.rerun()
